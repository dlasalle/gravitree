...
UNITTEST(OrbitalState, fromVectorsPerihelion)
{
  ...
  testNearEqual(state.orbit().parentMass(), 1.9885e30, 1.0e-8, 0.0);
  testNearEqual(state.orbit().semimajorAxis(), 1.496e11, 1.0e-3, 1.0);
  testNearEqual(state.orbit().eccentricity(), 0.01671, 1.0e-3, 1.0e-5);
  testNearEqual(state.orbit().inclination(), 0.0, 1.0e-3, 1.0e-2);
  testNearEqual(state.orbit().longitudeOfAscendingNode(), 0.0, 1.0e-3, \
      1.0e-3);
  testNearEqual(state.orbit().argumentOfPeriapsis(), 0.0, 1.0e-3, 1.0e-3);
  testNearEqual(state.orbit().periapsis(), 1.47095e11, 1.0e-3, 1.0);
  testNearEqual(state.orbit().apoapsis(), 1.521e11, 1.0e-3, 1.0);
  testNearEqual(state.orbit().period(), 365.25*24*60*60, 1.0e-3, 1.0e-2);
...
UNITTEST(OrbtialState, velocityFromAphelion)
{
  KeplerOrbit orbit(1.496e11, 0.0167, deg2rad(7.155), deg2rad(-11.26064),
      deg2rad(114.20783), 1.9885e30);
  OrbitalState state(orbit, 0);
  ...
  testNearEqual(state.velocity().magnitude(), 2.929e4, 1.0e-3, 1.0);
}

UNITTEST(OrbtialState, velocityFromPerihelion)
{
  KeplerOrbit orbit(1.496e11, 0.0167, deg2rad(7.155), deg2rad(-11.26064),
      deg2rad(114.20783), 1.9885e30);
  OrbitalState state(orbit, 0);
